<div wire:init="loadRegistros">
    <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                        ...
                        <tr>
                            <tr>
                                <td class="px-6 py-4 text-sm text-gray-800 text-left">
                                    {{ $movimiento->mostrarFecha() }}
                                </td>
                                <td class="px-6 py-4 text-sm text-gray-800">
                                    <p class="font-semibold">{{ $movimiento }}</p>
                                </td>
                                ...
                                <td class="py-4 text-right w-1/6 ">
                                    <button onclick="window.location.href='{{ route('movimientos.edit', $movimiento->id) }}'" 
                                    ...
                                    <button
                                        wire:click="$emit('prueba', {{ $movimiento->id }})" 
        ...
        @else
            @if ($readyToLoad)
                <div class="px-8 py-4">No hay movimientos a mostrar</div>
            @else
...
@push('scripts')
    ...
    <script>
        Livewire.on('prueba', movimientoId => {
            Swal.fire({
                title: "¿Eliminar el movimiento?",
                text: "Un movimiento eliminado no se puede recuperar",
                icon: "warning",
            ...
            if (result.isConfirmed) {
                // eliminarla  el movcimientos del  servidor
                Livewire.emit('eliminarMovimiento', movimientoId)
                Swal.fire({
                    title: "Se elimino el movimiento",
